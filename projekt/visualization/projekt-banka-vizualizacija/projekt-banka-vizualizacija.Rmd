...
```
Histogram za CPI i stambeni kredit:
```{r}
...
```
Scatter plot za CCI i housing
```{r}
boxplot(BankData$cons.conf.idx~BankData$housing,
        main = 'Boxplot of CCI and hosuing loan')
```
Po danom box plotu ne možemo najbolje vidjeti razliku.. pa za sada nećemo donositi nikakve zaključke dok ne testiramo u pythonu.

Nakon svih ovih testiranja, nismo primjetili neke velike sličnosti u varijablama. Probat ćemo sada kombinirati te varijable s varijablom subscribed koja govori je li se određena osoba pretplatila nakon obavljenog poziva.

Inače, naš je dataset napravljen na temelju anketiranja čiji je cilj "natjerati" ispitanika da se "pretplati".

Subscribed varijbla i CPI:

Podijelit ćemo dataset na one koji su se prijavili i one koji nisu:
```{r}
subscribed = BankData[which(BankData$subscribed == 1),]
not_subscribed = BankData[which(BankData$subscribed == 0),]
```

Sada promatramo prosječni CPI za one koji su se pretplatili, tj one koji nisu:
```{r}
...
```
Nakon toga prikazat ćemo box plot:
0 - not subscribed
1- subscribed
```{r}
...
```
Na box plotu se da primjetiti da je niži CPI kod ljudi koji su se na kraju pretplatili.

Sada idemo vidjeti vrijedi li isto i za CCI, Consumer Confiedence Index

Sada promatramo prosječni CCI za one koji su se pretplatili, tj one koji nisu:
```{r}
...
```
Ovdje pak vidimo da je kod onih koji su se pretplatili veći CCI, ali također i puno veća odstupanja..

Sada možemo gledati prosjeke ljudi koji su se pretplatili ovisno o edukaciji, godinama...
...
```
Ovdje je očito da je najviše pretplaćeno mladih ljudi.
...
Ovdje možemo vidjeti zanimljivost da je najviše pretplaćenih ljudi koji su nepismeni..
...
pie(x, labels, main = "Pie chart of subscribed people")
```
Možemo vidjeti da je više neoženjenih ili rastavljenih pretplaćeno.
